// --- START OF script.js (MODIFIED FOR ITEM DETAIL VIEW NAVIGATION + HUBCLEOUD & GDFLIX BYPASS + URL SPACE ENCODING + TMDB INTEGRATION - CORRECTED BYPASS/TMDB PERSISTENCE + ORIGINAL LINK VISIBILITY FIX + FETCH API FIX + INITIAL LOAD LIMIT FIX) ---
(function() {
    'use strict';
    ...
    const config = {
        ...
        INITIAL_SUGGESTION_FETCH_LIMIT: 300 // <<< REDUCED LIMIT FOR INITIAL LOAD
    };
    ...
    function preprocessMovieData(movie) {
        ...
        processed.tmdbDetails = null;

        // --- Title/Year/Season Extraction Logic ---
    ...
    function createMovieTableRowHTML(movie, dataIndex) {
        ...
        if (displayQuality === '4K' || lowerFilename.includes('2160p') || lowerFilename.includes('.4k.')) { fourkLogoHtml = `<img src="${config.FOURK_LOGO_URL}" alt="4K" class="quality-logo fourk-logo" title="4K Ultra HD" />`; }
        if ((displayQuality || '').includes('HDR') || (displayQuality || '').includes('DOLBY VISION') || displayQuality === 'DV' || lowerFilename.includes('hdr') || lowerFilename.includes('dolby.vision') || lowerFilename.includes('.dv.')) { hdrLogoHtml = `<img src="${config.HDR_LOGO_URL}" alt="HDR/DV" class="quality-logo hdr-logo" title="HDR / Dolby Vision Content" />`; }
        return `<tr class="movie-data-row" data-index="${dataIndex}" data-item-id="${sanitize(movie.id)}"><td class="col-id">${sanitize(movie.id || 'N/A')}</td><td class="col-filename" title="View details for: ${displayFilename}" data-item-id="${sanitize(movie.id)}">${displayFilename}${fourkLogoHtml}${hdrLogoHtml}</td><td class="col-size">${displaySize}</td><td class="col-quality">${displayQuality}</td><td class="col-updated" title="${formattedDateFull}">${formattedDateRelative}</td><td class="col-view"><button class="button view-button" data-item-id="${sanitize(movie.id)}">View</button></td></tr>`;
    }
    ...
    function setViewMode(mode) {
        ...
        if (showHomepage) {
            ...
            else if (localSuggestionData.length > 0) { if (updatesPreviewList) updatesPreviewList.innerHTML = '<div class="status-message" style="text-align:center; padding: 15px 0;">No recent updates found.</div>'; if (showMoreUpdatesButton) showMoreUpdatesButton.style.display = 'none'; }
            else { if (updatesPreviewList) updatesPreviewList.innerHTML = `<div class="loading-inline-spinner" role="status" aria-live="polite"><div class="spinner"></div><span>Loading updates...</span></div>`; }
            document.title = "Cinema Ghar Index";
        } else if (showSearch) { currentItemDetailData = null; isShareMode = false; document.title = "Cinema Ghar Index"; }
        saveStateToLocalStorage();
        isInitialLoad = false;
    }
    window.resetToHomepage = function(event) {
        console.log("Resetting to homepage...");
        if (window.history.pushState) { const cleanUrl = window.location.origin + window.location.pathname; window.history.pushState({ path: cleanUrl }, '', cleanUrl); }
        else { window.location.hash = ''; }
        isShareMode = false; currentItemDetailData = null; lastFocusedElement = event?.target;
        setViewMode('homepage');
        if (searchInput) { setTimeout(() => searchInput.focus(), 100); }
    }
    window.goBackToResults = function() {
        console.log("Navigating back from item detail view...");
        currentItemDetailData = null; isShareMode = false; history.back();
    }
    window.addEventListener('popstate', (event) => { console.log("Popstate event triggered", event.state); handleUrlChange(true); });
    function handleUrlChange(isPopState = false) {
        ...
        if (shareId) { if (currentViewMode !== 'itemDetail' || currentItemDetailData?.id !== shareId) { displayItemDetail(shareId, true); } }
        else if (viewId) { if (currentViewMode !== 'itemDetail' || currentItemDetailData?.id !== viewId) { displayItemDetail(viewId, false); } }
        else { if (currentViewMode !== 'homepage' && currentViewMode !== 'search') { setViewMode('homepage'); } else if (isInitialLoad) { setViewMode('homepage'); } }
        isInitialLoad = false;
    }
    ...
    window.handleSearchSubmit = function() { if (suggestionsContainer) { suggestionsContainer.style.display = 'none'; } const searchTerm = searchInput.value.trim(); console.log("Handling search submit for:", searchTerm); if (searchInput) { searchInput.blur(); } if (searchTerm.length === 0 && currentViewMode !== 'homepage') { resetToHomepage(); return; } if (searchTerm.length === 0 && currentViewMode === 'homepage') { return; } if (currentViewMode === 'itemDetail') { const cleanUrl = window.location.origin + window.location.pathname; history.pushState({ path: cleanUrl }, '', cleanUrl); } setViewMode('search'); activeResultsTab = 'allFiles'; currentState.currentPage = 1; currentState.searchTerm = searchTerm; currentState.qualityFilter = qualityFilterSelect.value || ''; currentState.typeFilter = ''; updateActiveTabAndPanel(); showLoadingStateInTables(`Searching for "${sanitize(searchTerm)}"...`); fetchAndRenderResults(); }
    function handleSearchClear() { clearTimeout(suggestionDebounceTimeout); suggestionsContainer.style.display = 'none'; setTimeout(() => { if (searchInput.value.trim() === '') { if (currentViewMode === 'search') { console.log("Search input cleared via 'x' while in search view, resetting to homepage."); resetToHomepage(); } else { currentState.searchTerm = ''; saveStateToLocalStorage(); } } }, 100); }
    ...
    window.appendMoreUpdates = async function() { if (!updatesPreviewList || !showMoreUpdatesButton) return; showMoreUpdatesButton.disabled = true; showMoreUpdatesButton.textContent = "Loading..."; const currentPage = Math.floor(updatesPreviewShownCount / config.UPDATES_PREVIEW_LOAD_MORE_COUNT); const nextPage = currentPage + 1; console.log(`Attempting to load page ${nextPage} for updates preview (currently showing ${updatesPreviewShownCount}).`); try { const params = { sort: 'lastUpdated', sortDir: 'desc', limit: config.UPDATES_PREVIEW_LOAD_MORE_COUNT, page: nextPage }; const data = await fetchApiData(params); if (data && data.items && data.items.length > 0) { const newItems = data.items.map(preprocessMovieData); const startIndex = weeklyUpdatesData.length; weeklyUpdatesData.push(...newItems); appendUpdatesToPreview(startIndex, weeklyUpdatesData.length); updatesPreviewShownCount += newItems.length; console.log(`Loaded ${newItems.length} more updates. Total now showing: ${updatesPreviewShownCount}. Current API page fetched: ${data.page}, Total API pages: ${data.totalPages}`); if (data.page >= data.totalPages || newItems.length < config.UPDATES_PREVIEW_LOAD_MORE_COUNT) { showMoreUpdatesButton.textContent = "All Updates Shown"; showMoreUpdatesButton.disabled = true; } else { showMoreUpdatesButton.disabled = false; showMoreUpdatesButton.textContent = "Show More"; } } else { console.log("No more updates found from API."); showMoreUpdatesButton.textContent = "No More Updates"; showMoreUpdatesButton.disabled = true; } } catch (error) { console.error("Failed to load more updates:", error); showMoreUpdatesButton.textContent = "Error Loading"; showMoreUpdatesButton.disabled = false; } }
    function appendUpdatesToPreview(startIndex, endIndex) { if (!updatesPreviewList) return; const fragment = document.createDocumentFragment(); const itemsToAppend = weeklyUpdatesData.slice(startIndex, endIndex); itemsToAppend.forEach((movie, indexInSlice) => { const overallIndex = startIndex + indexInSlice; if (!movie || !movie.id) return; const itemDiv = document.createElement('div'); itemDiv.className = 'update-item'; itemDiv.dataset.index = overallIndex; itemDiv.dataset.itemId = sanitize(movie.id); let hdrLogoHtml = ''; let fourkLogoHtml = ''; const lowerFilename = (movie.displayFilename || '').toLowerCase(); if (movie.displayQuality === '4K' || lowerFilename.includes('2160p') || lowerFilename.includes('.4k.')) { fourkLogoHtml = `<img src="${config.FOURK_LOGO_URL}" alt="4K" class="quality-logo fourk-logo" title="4K Ultra HD" />`; } if ((movie.displayQuality || '').includes('HDR') || (movie.displayQuality || '').includes('DOLBY VISION') || movie.displayQuality === 'DV' || lowerFilename.includes('hdr') || lowerFilename.includes('dolby.vision') || lowerFilename.includes('.dv.')) { hdrLogoHtml = `<img src="${config.HDR_LOGO_URL}" alt="HDR/DV" class="quality-logo hdr-logo" title="HDR / Dolby Vision Content" />`; } const timestampString = movie.last_updated_ts; const formattedDateRelative = TimeAgo.format(timestampString); const dateObject = timestampString ? new Date(timestampString) : null; const formattedDateFull = (dateObject && !isNaN(dateObject)) ? TimeAgo.formatFullDate(dateObject) : 'N/A'; itemDiv.innerHTML = `<div class="preview-col-id" title="ID: ${sanitize(movie.id || 'N/A')}">${sanitize(movie.id || 'N/A')}</div><div class="preview-col-filename" title="View details for: ${movie.displayFilename}" data-item-id="${sanitize(movie.id)}">${sanitize(movie.displayFilename)}${fourkLogoHtml}${hdrLogoHtml}</div><div class="preview-col-date" title="${formattedDateFull}">${formattedDateRelative}</div><div class="preview-col-view"><button class="button view-button" data-item-id="${sanitize(movie.id)}">View</button></div>`; fragment.appendChild(itemDiv); }); const initialLoader = updatesPreviewList.querySelector('.loading-inline-spinner'); if (initialLoader && startIndex === 0) { initialLoader.remove(); } updatesPreviewList.appendChild(fragment); }
    ...
    window.switchTab = function(tabId) { if (currentViewMode !== 'search' || tabId === activeResultsTab || !tabMappings[tabId]) { return; } activeResultsTab = tabId; currentState.currentPage = 1; currentState.typeFilter = tabMappings[tabId].typeFilter; closePlayerIfNeeded(null); updateActiveTabAndPanel(); showLoadingStateInTables(`Loading ${tabMappings[tabId].typeFilter || 'all files'}...`); fetchAndRenderResults(); saveStateToLocalStorage(); }

    // --- Navigation to Item Detail View ---
    function navigateToItemView(itemId) { if (!itemId) { console.error("Cannot navigate: Item ID is missing."); return; } console.log(`Navigating to view item: ${itemId}`); lastFocusedElement = document.activeElement; const newUrl = `${window.location.origin}${window.location.pathname}?viewId=${encodeURIComponent(itemId)}`; try { const currentParams = new URLSearchParams(window.location.search); if(currentParams.get('viewId') !== itemId) { history.pushState({ viewId: itemId }, '', newUrl); } else { console.log("Already on the correct viewId URL, not pushing state."); } displayItemDetail(itemId, false); } catch (e) { console.error("History pushState failed:", e); } }
    ...
    async function displayItemDetail(itemId, isFromShareLink) {
         ...
         try {
             ...
             if (!internalData) { console.error(`Failed to get internal data for ID: ${itemId}. Fetch might have been aborted or failed.`); itemDetailContent.innerHTML = `<div class="error-message" role="alert">Error: Could not retrieve data for item ID ${sanitize(itemId)}. The request may have been interrupted or failed. Please try again.</div>`; document.title = "Error Loading Item - Cinema Ghar Index"; return; }
             ...
             if (internalData && internalData.items && internalData.items.length > 0) {
                 ...
                 if (currentItemDetailData.extractedTitle) {
                     ...
                     try {
                         ...
                         if (tmdbResponse.ok) { currentItemDetailData.tmdbDetails = await tmdbResponse.json(); console.log("TMDb details fetched and stored successfully:", currentItemDetailData.tmdbDetails); }
                         else { const errorBody = await tmdbResponse.text(); console.warn(`Failed to fetch TMDb details (${tmdbResponse.status}): ${errorBody}`); currentItemDetailData.tmdbDetails = null; }
                     } catch (tmdbError) { clearTimeout(tmdbTimeoutId); currentItemDetailData.tmdbDetails = null; if (tmdbError.name === 'AbortError') { console.warn(`TMDb fetch timed out or aborted for: ${currentItemDetailData.extractedTitle}`); } else { console.error("Error fetching TMDb details:", tmdbError); } }
                 } else { console.warn("Cannot fetch TMDb details: No extracted title found."); currentItemDetailData.tmdbDetails = null; }
             ...
             } else { console.error(`Item not found via API for ID: ${itemId}`); itemDetailContent.innerHTML = `<div class="error-message" role="alert">Error: Item with ID ${sanitize(itemId)} was not found. It might have been removed or the link is incorrect.</div>`; document.title = "Item Not Found - Cinema Ghar Index"; }
         } catch (error) { console.error("Failed to fetch or display item detail:", error); itemDetailContent.innerHTML = `<div class="error-message" role="alert">Error loading item: ${error.message}. Please try again.</div>`; document.title = "Error Loading Item - Cinema Ghar Index"; currentItemDetailData = null; }
         finally { setViewMode('itemDetail'); window.scrollTo({ top: 0, behavior: 'smooth' }); if (pageLoader && pageLoader.style.display !== 'none') { pageLoader.style.display = 'none'; } }
     }

    // --- Bypass Update Function ---
    function updateItemDetailAfterBypass(encodedFinalUrl) {
          if (!currentItemDetailData || !itemDetailContent) { console.error("Cannot update item detail view: missing data or container."); return; }
          ...
          const playButton = itemDetailContent.querySelector('.play-button'); if(playButton) { setTimeout(() => playButton.focus(), 50); }
          if (videoContainer.parentElement && videoContainer.style.display !== 'none') { console.log("Re-attaching player to updated item detail content."); const separator = itemDetailContent.querySelector('hr.detail-separator'); const targetElement = separator ? separator.nextElementSibling : itemDetailContent.firstChild; if (targetElement && targetElement.nextSibling) { itemDetailContent.insertBefore(videoContainer, targetElement.nextSibling); } else { itemDetailContent.appendChild(videoContainer); } }
     }

    // --- Player Logic ---
    function streamVideo(title, url, filenameForAudioCheck, isFromCustom = false) {
        let currentActionContainer = null;
        if (isGlobalCustomUrlMode) { /* Player stays global */ }
        else if (currentViewMode === 'itemDetail' && itemDetailContent) { currentActionContainer = itemDetailContent; }
        else { console.warn("Cannot determine where to place the video player, using itemDetailContent fallback."); if (itemDetailContent) { currentActionContainer = itemDetailContent; } else { console.error("Cannot stream video: No valid container found."); return; } }
    ...
    async function fetchApiData(params = {}) {
        // Build query string from params object, ensuring correct encoding
        ...
        const url = `${config.MOVIE_DATA_API_URL}?${query.toString()}`; // Use toString() for proper encoding

        if (searchAbortController && searchAbortController !== localAbortController) {
            console.log(`[fetchApiData] Aborting previous request controller signal:`, searchAbortController.signal);
            searchAbortController.abort(`New request started for ${url}`);
        }
        searchAbortController = localAbortController;

        console.log(`[fetchApiData] Attempting fetch for: ${url}`);
        console.log(`[fetchApiData] Using signal:`, signal);

        let response;

        try {
            console.log(`[fetchApiData] >>> EXECUTING await fetch for ${url}`);
            response = await fetch(url, { signal });
            console.log(`[fetchApiData] <<< COMPLETED await fetch for ${url}. Response variable holds:`, response);

            if (!response) {
                 console.error(`!!! [fetchApiData] CRITICAL: FETCH API call to ${url} returned a non-response value. Value was:`, response);
                 throw new Error(`API Error: Fetch for ${url} did not return a valid response object.`);
            }

            console.log(`[fetchApiData] Response received for ${url}. Status: ${response.status}, OK: ${response.ok}`);
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    errorDetails = errorBody?.error || errorBody?.details || errorDetails;
                    console.warn(`[fetchApiData] API Error Response (JSON) from ${url}:`, errorBody);
                } catch (jsonError) {
                    console.warn(`[fetchApiData] Failed to parse error response as JSON from ${url} (Status: ${response.status}). Trying text...`);
                    try { const errorText = await response.text(); errorDetails = errorText || errorDetails; console.warn(`[fetchApiData] API Error Response (Text) from ${url}:`, errorText); }
                    catch (textError) { console.error(`[fetchApiData] Failed to read error response as text from ${url}:`, textError); }
                }
                throw new Error(`API Error: ${errorDetails}`); // Throw consolidated error
            }

            const data = await response.json();
            console.log(`[fetchApiData] API data parsed successfully for ${url}:`, data);

            if (!params.id && tabMappings[activeResultsTab]) { // Update pagination data only for list views
                const activePagination = tabMappings[activeResultsTab]?.pagination;
                if (activePagination && data.totalPages !== undefined) { activePagination.dataset.totalPages = data.totalPages; }
            }
            return data;

        } catch (error) {
            console.error(`!!! [fetchApiData] ERROR CAUGHT for URL: ${url} !!!`);
            console.error("[fetchApiData] - Error Type:", error?.name);
            console.error("[fetchApiData] - Error Message:", error?.message);

            if (error.name === 'AbortError') {
                console.log(`[fetchApiData] API fetch aborted for ${url}. Reason:`, signal.reason);
                return null;
            }
            throw error; // Re-throw other errors
        } finally {
            if (searchAbortController === localAbortController) {
                searchAbortController = null;
                console.log(`[fetchApiData] Cleared searchAbortController reference after finishing request for ${url}`);
            } else {
                 console.log(`[fetchApiData] Did NOT clear searchAbortController reference after finishing ${url} (a newer request is active)`);
            }
        }
    }
    ...
    async function initializeApp() {
        ...
        const isLoadingDetailView = initialUrlParams.has('viewId') || initialUrlParams.has('shareId');

        handleUrlChange(); // Setup initial view (homepage or detail)

        try {
            // Fetch suggestion/quality data in the background *only if not loading detail view initially*
            if (!isLoadingDetailView) {
                 console.log("[initializeApp] Not loading detail view initially, fetching initial data for suggestions/quality filter...");
                 // --- Fetch with REDUCED LIMIT ---
                 fetchApiData({ limit: config.INITIAL_SUGGESTION_FETCH_LIMIT, sort: 'lastUpdated', sortDir: 'desc' })
                    .then(suggestionData => {
                        if (suggestionData && suggestionData.items) {
                            localSuggestionData = suggestionData.items.map(preprocessMovieData);
                            console.log(`[initializeApp] Loaded ${localSuggestionData.length} items for suggestions.`);
                            populateQualityFilter(localSuggestionData);
                            if (currentViewMode === 'homepage') {
                                weeklyUpdatesData = localSuggestionData;
                                displayInitialUpdates();
                            }
                        } else {
                            console.warn("[initializeApp] No valid initial data received for suggestions/quality filter.");
                    ...
                    }).catch(e => { // Catch errors specifically from this fetch chain
                        console.error("[initializeApp] Background suggestion/quality/updates fetch failed:", e);
                         if (currentViewMode === 'homepage' && updatesPreviewList) {
                 ...
                 console.log("[initializeApp] Loading detail view initially, skipping background suggestion fetch.");
                 // TODO: Consider fetching quality filter data separately if needed on detail view load
            }
        ...
        } catch (error) { // Catch errors during initial state loading etc.
            console.error('FATAL: Failed during app initialization phase:', error);
    ...
    function handleActionClick(event) {
         ...
         if (button) {
            ...
            let title = button.dataset.title || currentItemDetailData?.displayFilename;
            ...
            if (button.tagName === 'A' && button.href && button.target === '_blank') { return; }
            event.preventDefault();
            console.log(`Action clicked: ${action}`);
            if (action === 'play' && url) { isGlobalCustomUrlMode = false; streamVideo(title, url, filename); }
            else if (action === 'copy-vlc' && url) { copyVLCLink(button, url); }
            else if (action === 'open-intent' && url) { openWithIntent(url); }
            else if (action === 'share' && id) { handleShareClick(button); }
            else if (action === 'toggle-custom-url') { toggleCustomUrlInput(button); }
            else if (action === 'bypass-hubcloud') { triggerHubCloudBypass(button); }
            else if (action === 'bypass-gdflix') { triggerGDFLIXBypass(button); }
            else if (target.matches('#playerPlayCustomUrlButton')) { if (isGlobalCustomUrlMode) { handleGlobalPlayCustomUrl(event); } else { playFromCustomUrlInput(event.target); } }
         }
    }
    function handleGlobalCustomUrlClick(event) {
         event.preventDefault(); lastFocusedElement = event.target; if (!videoContainer || !playerCustomUrlSection || !playerCustomUrlInput) return; console.log("Global Play Custom URL clicked."); closePlayerIfNeeded(); if(resultsArea) resultsArea.style.display = 'none'; if(itemDetailView) itemDetailView.style.display = 'none'; if(searchFocusArea) searchFocusArea.style.display = 'none'; if(pageFooter) pageFooter.style.display = 'none'; isGlobalCustomUrlMode = true; videoContainer.classList.add('global-custom-url-mode'); if (videoElement) videoElement.style.display = 'none'; if (customControlsContainer) customControlsContainer.style.display = 'none'; if (videoTitle) videoTitle.innerText = 'Play Custom URL'; if (vlcBox) vlcBox.style.display = 'none'; if (audioWarningDiv) audioWarningDiv.style.display = 'none'; playerCustomUrlSection.style.display = 'flex'; playerCustomUrlInput.value = ''; if (playerCustomUrlFeedback) playerCustomUrlFeedback.textContent = ''; videoContainer.style.display = 'flex'; setTimeout(() => playerCustomUrlInput.focus(), 50);
    }
    function handleGlobalPlayCustomUrl(event) {
         event.preventDefault(); if (!playerCustomUrlInput || !playerCustomUrlFeedback) return; const customUrlRaw = playerCustomUrlInput.value.trim(); playerCustomUrlFeedback.textContent = ''; if (!customUrlRaw) { playerCustomUrlFeedback.textContent = 'Please enter a URL.'; playerCustomUrlInput.focus(); return; } let customUrlEncoded = customUrlRaw; try { new URL(customUrlRaw); customUrlEncoded = customUrlRaw.replace(/ /g, '%20'); } catch (e) { playerCustomUrlFeedback.textContent = 'Invalid URL format.'; playerCustomUrlInput.focus(); return; } console.log(`Attempting to play global custom URL: ${customUrlEncoded}`); if(playerCustomUrlSection) playerCustomUrlSection.style.display = 'none'; if(videoElement) videoElement.style.display = 'block'; if(customControlsContainer) customControlsContainer.style.display = 'flex'; streamVideo("Custom URL Video", customUrlEncoded, null, true);
    }
    function toggleCustomUrlInput(toggleButton, triggeredByError = false) {
         const contextContainer = toggleButton.closest('#item-detail-content') || toggleButton.closest('#videoContainer'); if (!contextContainer || !videoContainer || !playerCustomUrlSection) { console.error("Cannot toggle custom URL input: context or player elements missing."); return; }
         if (contextContainer.id === 'item-detail-content' && videoContainer.parentElement !== contextContainer) { console.warn("Player not in item detail container, moving it for custom URL toggle."); const separator = contextContainer.querySelector('hr.detail-separator'); const targetElement = separator ? separator.nextElementSibling : contextContainer.firstChild; if(videoContainer.parentElement) videoContainer.parentElement.removeChild(videoContainer); if (targetElement && targetElement.nextSibling) { contextContainer.insertBefore(videoContainer, targetElement.nextSibling); } else { contextContainer.appendChild(videoContainer); } if (videoElement && videoElement.hasAttribute('src')) { videoElement.pause(); videoElement.removeAttribute('src'); videoElement.currentTime = 0; videoElement.load(); } if (vlcBox) vlcBox.style.display = 'none'; if (audioWarningDiv) audioWarningDiv.style.display = 'none'; if (audioTrackSelect) { audioTrackSelect.innerHTML = ''; audioTrackSelect.style.display = 'none'; } clearCopyFeedback(); }
         const isHidden = playerCustomUrlSection.style.display === 'none'; playerCustomUrlSection.style.display = isHidden ? 'flex' : 'none'; videoElement.style.display = isHidden ? 'none' : 'block'; customControlsContainer.style.display = isHidden ? 'none' : 'flex'; if(vlcBox) vlcBox.style.display = isHidden ? 'none' : 'block';
         if(audioWarningDiv) { if (isHidden && audioWarningDiv.style.display !== 'none' && !audioWarningDiv.innerHTML.includes('Playback Error:')) { audioWarningDiv.style.display = 'none'; } else if (!isHidden && audioWarningDiv.style.display === 'none') { const movieData = currentItemDetailData; if (movieData && movieData.displayFilename) { const ddp51Regex = /\bDDP?([ ._-]?5\.1)?\b/i; const advancedAudioRegex = /\b(DTS|ATMOS|TrueHD)\b/i; const multiAudioHintRegex = /\b(Multi|Dual)[ ._-]?Audio\b/i; let warningText = ""; const lowerFilename = movieData.displayFilename.toLowerCase(); if (ddp51Regex.test(lowerFilename)) { warningText = "<strong>Audio Note:</strong> DDP audio might not work in browser. Use 'Copy URL' or 'Play in VLC or MX Player'."; } else if (advancedAudioRegex.test(lowerFilename)) { warningText = "<strong>Audio Note:</strong> DTS/Atmos/TrueHD audio likely unsupported. Use external player."; } else if (multiAudioHintRegex.test(lowerFilename)) { warningText = "<strong>Audio Note:</strong> May contain multiple audio tracks. Use selector below or external player."; } if(warningText) { audioWarningDiv.innerHTML = warningText; audioWarningDiv.style.display = 'block'; } } } }
         if (videoContainer.style.display === 'none') { videoContainer.style.display = 'flex'; } toggleButton.setAttribute('aria-expanded', String(isHidden)); toggleButton.innerHTML = isHidden ? '<span aria-hidden="true">🔼</span> Hide Custom URL Input' : '<span aria-hidden="true">🔗</span> Play Custom URL';
         if (isHidden && !triggeredByError) { if (playerCustomUrlInput) setTimeout(() => playerCustomUrlInput.focus(), 50); } else if (!isHidden) { setTimeout(() => toggleButton.focus(), 50); } setTimeout(() => { videoContainer.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); }, 150);
    }
    function playFromCustomUrlInput(playButton) {
         const container = playButton.closest('#playerCustomUrlSection'); if (!container) return; const inputField = container.querySelector('#playerCustomUrlInput'); const feedbackSpan = container.querySelector('.player-custom-url-feedback'); const titleRef = "Custom URL Video"; if (!inputField || !feedbackSpan) return; const customUrlRaw = inputField.value.trim(); feedbackSpan.textContent = ''; if (!customUrlRaw) { feedbackSpan.textContent = 'Please enter a URL.'; inputField.focus(); return; } let customUrlEncoded = customUrlRaw; try { new URL(customUrlRaw); customUrlEncoded = customUrlRaw.replace(/ /g, '%20'); } catch (e) { feedbackSpan.textContent = 'Invalid URL format.'; inputField.focus(); return; } console.log(`Attempting to play custom URL from item context: ${customUrlEncoded}`); isGlobalCustomUrlMode = false; if (playerCustomUrlSection) playerCustomUrlSection.style.display = 'none'; if (videoElement) videoElement.style.display = 'block'; if (customControlsContainer) customControlsContainer.style.display = 'flex'; streamVideo(titleRef, customUrlEncoded, null, true);
    }

    // --- HubCloud/GDFLIX Bypass Logic ---
    async function triggerHubCloudBypass(buttonElement) { const hubcloudUrl = buttonElement.dataset.hubcloudUrl; const movieRefType = buttonElement.dataset.movieRef; if (!hubcloudUrl) { console.error("Bypass failed: HubCloud URL missing from button data."); setBypassButtonState(buttonElement, 'error', 'Missing URL'); return; } if (movieRefType !== 'detail' || !currentItemDetailData) { console.error("Bypass failed: Invalid context or missing item data."); setBypassButtonState(buttonElement, 'error', 'Context Error'); return; } console.log(`Attempting HubCloud bypass for: ${hubcloudUrl} (Context: ${movieRefType})`); setBypassButtonState(buttonElement, 'loading'); const apiController = new AbortController(); const timeoutId = setTimeout(() => { apiController.abort(); console.error(`HubCloud Bypass API call timed out after ${config.BYPASS_TIMEOUT / 1000}s`); setBypassButtonState(buttonElement, 'error', 'Timeout'); }, config.BYPASS_TIMEOUT); try { const response = await fetch(config.BYPASS_API_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ hubcloudUrl }), signal: apiController.signal }); clearTimeout(timeoutId); if (!response.ok) { let errorDetails = `HTTP Error: ${response.status}`; try { errorDetails = (await response.json()).details || errorDetails; } catch (_) {} throw new Error(errorDetails); } const result = await response.json(); if (result.success && result.finalUrl) { console.log(`HubCloud Bypass successful! Raw Final URL: ${result.finalUrl}`); const encodedFinalUrl = result.finalUrl.replace(/ /g, '%20'); console.log(`Encoded Final URL: ${encodedFinalUrl}`); setBypassButtonState(buttonElement, 'success', 'Success!'); updateItemDetailAfterBypass(encodedFinalUrl); } else { throw new Error(result.details || result.error || 'Unknown HubCloud bypass failure'); } } catch (error) { clearTimeout(timeoutId); if (error.name === 'AbortError' && !apiController.signal.aborted) { console.error("HubCloud Bypass aborted due to timeout."); setBypassButtonState(buttonElement, 'error', 'Timeout'); } else if (error.name === 'AbortError') { console.log("HubCloud Bypass fetch aborted by user/navigation."); setBypassButtonState(buttonElement, 'idle'); } else { console.error("HubCloud Bypass failed:", error); setBypassButtonState(buttonElement, 'error', `Failed: ${error.message.substring(0, 50)}`); } } }
    async function triggerGDFLIXBypass(buttonElement) { const gdflixUrl = buttonElement.dataset.gdflixUrl; const movieRefType = buttonElement.dataset.movieRef; if (!gdflixUrl) { console.error("Bypass failed: GDFLIX URL missing from button data."); setBypassButtonState(buttonElement, 'error', 'Missing URL'); return; } if (movieRefType !== 'detail' || !currentItemDetailData) { console.error("Bypass failed: Invalid context or missing item data."); setBypassButtonState(buttonElement, 'error', 'Context Error'); return; } console.log(`Attempting GDFLIX bypass for: ${gdflixUrl} (Context: ${movieRefType})`); setBypassButtonState(buttonElement, 'loading'); const apiController = new AbortController(); const timeoutId = setTimeout(() => { apiController.abort(); console.error(`GDFLIX Bypass API call timed out after ${config.BYPASS_TIMEOUT / 1000}s`); setBypassButtonState(buttonElement, 'error', 'Timeout'); }, config.BYPASS_TIMEOUT); try { const response = await fetch(config.GDFLIX_BYPASS_API_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ gdflixUrl }), signal: apiController.signal }); clearTimeout(timeoutId); if (!response.ok) { let errorDetails = `HTTP Error: ${response.status}`; try { errorDetails = (await response.json()).error || errorDetails; } catch (_) {} throw new Error(errorDetails); } const result = await response.json(); if (result.success && result.finalUrl) { console.log(`GDFLIX Bypass successful! Raw Final URL: ${result.finalUrl}`); const encodedFinalUrl = result.finalUrl.replace(/ /g, '%20'); console.log(`Encoded Final URL: ${encodedFinalUrl}`); setBypassButtonState(buttonElement, 'success', 'Success!'); updateItemDetailAfterBypass(encodedFinalUrl); } else { throw new Error(result.error || 'Unknown GDFLIX bypass failure'); } } catch (error) { clearTimeout(timeoutId); if (error.name === 'AbortError' && !apiController.signal.aborted) { console.error("GDFLIX Bypass aborted due to timeout."); setBypassButtonState(buttonElement, 'error', 'Timeout'); } else if (error.name === 'AbortError') { console.log("GDFLIX Bypass fetch aborted by user/navigation."); setBypassButtonState(buttonElement, 'idle'); } else { console.error("GDFLIX Bypass failed:", error); setBypassButtonState(buttonElement, 'error', `Failed: ${error.message.substring(0, 50)}`); } } }
    function setBypassButtonState(buttonElement, state, message = null) { if (!buttonElement) return; const feedbackSpan = buttonElement.nextElementSibling; const iconSpan = buttonElement.querySelector('.button-icon'); const spinnerSpan = buttonElement.querySelector('.button-spinner'); const textSpan = buttonElement.querySelector('.button-text'); const isHubCloud = buttonElement.classList.contains('hubcloud-bypass-button'); const defaultText = isHubCloud ? 'Bypass HubCloud' : 'Bypass GDFLIX'; const defaultIconHTML = isHubCloud ? '☁️' : '🎬'; buttonElement.classList.remove('loading', 'error', 'success'); buttonElement.disabled = false; if (feedbackSpan) feedbackSpan.style.display = 'none'; if (spinnerSpan) spinnerSpan.style.display = 'none'; if (iconSpan) iconSpan.style.display = 'inline-block'; clearTimeout(bypassFeedbackTimeout); switch (state) { case 'loading': buttonElement.classList.add('loading'); buttonElement.disabled = true; if (textSpan) textSpan.textContent = 'Bypassing...'; if (spinnerSpan) spinnerSpan.style.display = 'inline-block'; if (iconSpan) iconSpan.style.display = 'none'; if (feedbackSpan) { feedbackSpan.textContent = 'Please wait...'; feedbackSpan.className = 'bypass-feedback loading show'; feedbackSpan.style.display = 'inline-block'; } break; case 'success': buttonElement.classList.add('success'); buttonElement.disabled = true; if (textSpan) textSpan.textContent = 'Success!'; if (iconSpan) iconSpan.innerHTML = '✅'; if (spinnerSpan) spinnerSpan.style.display = 'none'; if (iconSpan) iconSpan.style.display = 'inline-block'; if (feedbackSpan) { feedbackSpan.textContent = message || 'Success! Play button updated.'; feedbackSpan.className = 'bypass-feedback success show'; feedbackSpan.style.display = 'inline-block'; } break; case 'error': buttonElement.classList.add('error'); buttonElement.disabled = false; if (textSpan) textSpan.textContent = defaultText; if (iconSpan) iconSpan.innerHTML = defaultIconHTML; if (spinnerSpan) spinnerSpan.style.display = 'none'; if (iconSpan) iconSpan.style.display = 'inline-block'; if (feedbackSpan) { feedbackSpan.textContent = message || 'Failed'; feedbackSpan.className = 'bypass-feedback error show'; feedbackSpan.style.display = 'inline-block'; bypassFeedbackTimeout = setTimeout(() => { feedbackSpan.classList.remove('show', 'error', 'loading'); feedbackSpan.style.display = 'none'; feedbackSpan.textContent = ''; }, 4000); } break; case 'idle': default: buttonElement.disabled = false; if (textSpan) textSpan.textContent = defaultText; if (iconSpan) iconSpan.innerHTML = defaultIconHTML; if (spinnerSpan) spinnerSpan.style.display = 'none'; if (iconSpan) iconSpan.style.display = 'inline-block'; if (feedbackSpan) { feedbackSpan.classList.remove('show', 'error', 'loading'); feedbackSpan.style.display = 'none'; feedbackSpan.textContent = ''; } break; } }
    ...
    function handleContentClick(event) {
         ...
         if (viewTrigger) { event.preventDefault(); const itemId = viewTrigger.dataset.itemId || viewTrigger.closest('[data-item-id]')?.dataset.itemId; if (itemId) { navigateToItemView(itemId); } else { console.error("Could not find item ID for navigation."); } return; }
         const actionTrigger = target.closest('#item-detail-content .button, #videoContainer .button:not(.close-btn)');
         if (actionTrigger && !actionTrigger.closest('.custom-controls')) { handleActionClick(event); return; }
         if (target.matches('.close-btn') && target.closest('#videoContainer')) { lastFocusedElement = target; closePlayer(lastFocusedElement); return; }
    ...
    document.addEventListener('DOMContentLoaded', async () => {
         ...
         document.addEventListener('click', (event) => { if (searchInput && suggestionsContainer && suggestionsContainer.style.display === 'block') { const searchWrapper = searchInput.closest('.search-input-wrapper'); if (searchWrapper && !searchWrapper.contains(event.target)) { suggestionsContainer.style.display = 'none'; } } if (videoContainer && videoContainer.style.display !== 'none' && !isGlobalCustomUrlMode) { const clickedInsidePlayer = videoContainer.contains(event.target); const clickedInsideDetailContent = itemDetailContent?.contains(event.target); if (!clickedInsidePlayer && !clickedInsideDetailContent) { let triggerElement = lastFocusedElement; let clickedOnPotentialTrigger = triggerElement && event.target.closest('.button') === triggerElement; if (!clickedOnPotentialTrigger) { console.log("Clicked outside player and its detail view context. Closing player."); closePlayer(event.target); } } } else if (videoContainer && videoContainer.style.display !== 'none' && isGlobalCustomUrlMode) { const clickedInsidePlayer = videoContainer.contains(event.target); const clickedOnGlobalTrigger = playCustomUrlGlobalButton && playCustomUrlGlobalButton.contains(event.target); if (!clickedInsidePlayer && !clickedOnGlobalTrigger) { console.log("Clicked outside global player and its trigger. Closing player."); closePlayer(event.target); } } }, false);

// --- START OF script.js (MODIFIED FOR ITEM DETAIL VIEW + BYPASS + TMDB INTEGRATION) ---
(function() {
    'use strict';
    ...
    const config = {
        ...
        TMDB_API_PROXY_URL: '/api/tmdb', // <<< ADDED FOR TMDB >>>
        TMDB_FETCH_TIMEOUT: 15000 // Timeout for TMDb fetch (15 seconds) <<< ADDED FOR TMDB >>>
    };
    ...
    // --- Data Preprocessing (Handles HubCloud/GDFLIX links, encodes URL spaces, extracts title/year/season) ---
    function preprocessMovieData(movie) {
        ...
        if (filename) {
            // Try to find Season first (more reliable indicator for series)
            const seasonMatch = filename.match(/[.(_\-\s]S(\d{1,2})(?:E\d{1,3}|[.(_\-\s])/i);
            if (seasonMatch && seasonMatch[1]) {
                processed.extractedSeason = parseInt(seasonMatch[1], 10);
                processed.isSeries = true; // Mark as series if Sxx found
                const titleEndIndex = seasonMatch.index;
                 // Take text before Sxx, remove dots/underscores, remove year in brackets/parens at the end
                processed.extractedTitle = filename.substring(0, titleEndIndex)
                                                .replace(/[._]/g, ' ')
                                                .replace(/\s+/g, ' ')
                                                .replace(/\s+\(\d{4}\)$|\s+\[\d{4}\]$/, '') // Remove year at the end if present
                                                .trim();
                 // Try to find year separately for series title
                 const seriesYearMatch = processed.extractedTitle.match(/\(?(\d{4})\)?$/);
                 if(seriesYearMatch && seriesYearMatch[1]){
                      const year = parseInt(seriesYearMatch[1], 10);
                      if (year > 1900 && year < 2050) {
                          processed.extractedYear = year;
                          // Optionally remove the year from the title itself if needed, but often kept for series
                           // processed.extractedTitle = processed.extractedTitle.replace(/\s+\(?\d{4}\)?$/, '').trim();
                      }
                 }


            } else {
                 // Not a season match, treat as potential movie
                 processed.isSeries = false; // Assume movie
                 // Try to find year (more robust pattern)
                 const yearMatch = filename.match(/[.(_\-\s](\d{4})[.(_\-\s\]\)]/);
                 if (yearMatch && yearMatch[1]) {
                    const year = parseInt(yearMatch[1], 10);
                    if (year > 1900 && year < 2050) { // Basic sanity check for year
                        processed.extractedYear = year;
            ...
            // Fallback title extraction if year/season didn't yield one
            if (!processed.extractedTitle) {
                processed.extractedTitle = filename.split(/[\.({\[]/)[0].replace(/[._]/g, ' ').replace(/\s+/g, ' ').trim();
                // Basic cleanup for fallback title (remove trailing quality/codec)
                processed.extractedTitle = processed.extractedTitle
                                             .replace(/ (4K|2160p|1080p|720p|480p|WEB-?DL|BluRay|BDRip|BRRip|HDTV|HDRip|DVDrip|DVDScr|HDR|DV|HEVC|x265)$/i, '')
                                             .trim();
            }

            // Final cleanup on title
             if (processed.extractedTitle) {
                // Remove known release group names or common tags at the end
                processed.extractedTitle = processed.extractedTitle.replace(/ [-._\s](RARBG|TGx|YTS|EXTREME|EVO|PSA|GalaxyRG|PAHE|\[.*?\])$/i, '').trim();
             }
        }
        return processed;
    }
    ...
    function createItemDetailContentHTML(movie, tmdbDetails) { // Added tmdbDetails parameter
        ...
        let imdbSearchButtonHTML = ''; // Will hold IMDb or TMDb link button
        ...
        if (movie.extractedTitle) {
            ...
            youtubeTrailerButtonHTML = `<a href="${youtubeSearchUrl}" target="_blank" rel="noopener noreferrer" class="button youtube-button">${youtubeIconSVG} Watch Trailer</a>`;
             ...
             // IMDb / TMDb Link Button
             // Use placeholder SVG for now, can be replaced with actual logos if desired
             ...
             if (tmdbDetails && tmdbDetails.tmdbLink) {
                 // Use direct TMDb link if we fetched it
                  ...
                  imdbSearchButtonHTML = `<a href="${sanitize(tmdbDetails.tmdbLink)}" target="_blank" rel="noopener noreferrer" class="button tmdb-link-button">${infoIconSVG} ${tmdbLabel}</a>`;
             } else {
                 ...
                 imdbSearchButtonHTML = `<a href="${imdbSearchUrl}" target="_blank" rel="noopener noreferrer" class="button imdb-button">${infoIconSVG} Find on IMDb</a>`;
             }
         }
        ...
        if (tmdbDetails && tmdbDetails.id) {
            ...
            // Check if the view is currently active before showing the fetch failed message
            if (currentViewMode === 'itemDetail' && currentItemDetailData && currentItemDetailData.id === movie.id) {
        ...
        // 3. Other Link Buttons (Trailer, IMDb/TMDb, Original Links, Custom URL, Share)
        ...
        // Original Links (Show link only if bypass button isn't already present for that type)
    ...
    function handleUrlChange(isPopState = false) {
        ...
        if (shareId) {
            ...
            if (!isInitialLoad || isPopState) { // Consider popstate as navigation away from detail view
                console.log("Navigating back to homepage or search results view.");
                 // Determine if we should go to homepage or search
                 // Simplest: Always go to homepage when navigating back from detail via popstate
    ...
    window.closePlayer = function(elementToFocusAfter = null) {
         ...
         // --- IMPORTANT: Now we just detach, don't move back to main container ---
         if (!wasGlobalMode) {
    ...
    function populateAudioTrackSelector() { if (!videoElement || typeof videoElement.audioTracks === 'undefined' || !audioTrackSelect) { if(audioTrackSelect) audioTrackSelect.style.display = 'none'; return; } const tracks = videoElement.audioTracks; audioTrackSelect.innerHTML = ''; if (tracks.length <= 1) { audioTrackSelect.style.display = 'none'; return; } let hasEnabledTrack = false; for (let i = 0; i < tracks.length; i++) { if (tracks[i].enabled) hasEnabledTrack = true; } if (!hasEnabledTrack && tracks.length > 0) { try { tracks[0].enabled = true; } catch(e) { console.warn("Could not auto-enable first audio track:", e); } } let preferredTrackIndex = -1; // Find Hindi track index
         for (let i = 0; i < tracks.length; i++) {
             ...
             if (lang) {
                 try {
                     languageName = new Intl.DisplayNames(['en'], { type: 'language' }).of(lang.split('-')[0]).toLowerCase();
                 } catch (e) {}
             }
             // Check for Hindi explicitly
             if (lang?.startsWith('hi') || label.includes('hindi') || languageName === 'hindi') {
                 preferredTrackIndex = i;
                 break; // Found preferred track, stop searching
             }
         }
         // If no Hindi track found, try English
         if (preferredTrackIndex === -1) {
              for (let i = 0; i < tracks.length; i++) {
                 ...
                 const label = track.label?.toLowerCase() || '';
                 let languageName = '';
                 if (lang) { try { languageName = new Intl.DisplayNames(['en'], { type: 'language' }).of(lang.split('-')[0]).toLowerCase(); } catch(e) {} }
                 if (lang?.startsWith('en') || label.includes('english') || languageName === 'english') {
                     preferredTrackIndex = i;
                     break;
                 }
              }
         }
         // If still no preference, default to first enabled or just the first track
         if (preferredTrackIndex === -1) {
             for (let i = 0; i < tracks.length; i++) { if (tracks[i].enabled) { preferredTrackIndex = i; break; } }
             if (preferredTrackIndex === -1 && tracks.length > 0) preferredTrackIndex = 0; // Fallback to the very first track
         }

          // Enable the preferred track and disable others
         if (preferredTrackIndex !== -1) {
             console.log(`Preferred/Selected track index: ${preferredTrackIndex}. Attempting selection.`);
             try {
                 for (let i = 0; i < tracks.length; i++) {
                     tracks[i].enabled = (i === preferredTrackIndex);
                 }
                 console.log(`Successfully enabled track ${preferredTrackIndex}.`);
             } catch (e) {
                 console.error("Error auto-selecting preferred audio track:", e);
             }
         } else {
             console.log("No specific preferred track found or enabled.");
         }
         ...
         for (let i = 0; i < tracks.length; i++) {
             ...
             // Mark the currently enabled track as selected in the dropdown
         ...
         // Set dropdown value to the selected track
         if (preferredTrackIndex !== -1) {
              const preferredTrackValue = tracks[preferredTrackIndex].id || preferredTrackIndex;
              audioTrackSelect.value = preferredTrackValue;
         }

         try { if (tracks.onchange === null) tracks.onchange = populateAudioTrackSelector; } catch(e) { console.warn("Browser might not support 'onchange' on AudioTrackList", e)} }
    ...
    async function initializeApp() {
        ...
        try {
            ...
            // Only hide loader if not showing item detail (detail view hides it in its own finally block)
    ...
    function handleActionClick(event) { // Handles clicks within Item Detail View or Player
         ...
         if (button) {
            ...
            let title = button.dataset.title || button.closest('[data-stream-title]')?.dataset.streamTitle || currentItemDetailData?.displayFilename; // Get title
            ...
            if (button.tagName === 'A' && button.href && button.target === '_blank') {
             ...
             // Don't prevent default for the download link button
             if (!(button.tagName === 'A' && button.classList.contains('download-button'))) {
    ...
    async function triggerHubCloudBypass(buttonElement) {
         ...
         try {
             ...
             if (error.name === 'AbortError' && apiController.signal.aborted) { // Check if aborted by timeout
                 console.error("HubCloud Bypass aborted due to timeout.");
                 setBypassButtonState(buttonElement, 'error', 'Timeout'); // Show timeout error specifically
             } else if (error.name === 'AbortError') {
                 console.log("HubCloud Bypass fetch aborted by user/navigation.");
                 setBypassButtonState(buttonElement, 'idle'); // Reset if aborted by other means
             } else {
                 console.error("HubCloud Bypass failed:", error);
                 setBypassButtonState(buttonElement, 'error', `Failed: ${error.message.substring(0, 50)}`);
             }
         }
     }
    async function triggerGDFLIXBypass(buttonElement) { // New GDFLIX function
         ...
         try {
             ...
             clearTimeout(timeoutId);
              if (error.name === 'AbortError' && apiController.signal.aborted) { // Check if aborted by timeout
                 console.error("GDFLIX Bypass aborted due to timeout.");
                 setBypassButtonState(buttonElement, 'error', 'Timeout'); // Show timeout error specifically
             } else if (error.name === 'AbortError') {
                 console.log("GDFLIX Bypass fetch aborted by user/navigation.");
                 setBypassButtonState(buttonElement, 'idle'); // Reset if aborted by other means
             } else {
                 console.error("GDFLIX Bypass failed:", error);
                 setBypassButtonState(buttonElement, 'error', `Failed: ${error.message.substring(0, 50)}`);
             }
         }
     }
    function updateItemDetailAfterBypass(encodedFinalUrl) { // Reusable function for item detail view
          ...
          // Fetch TMDb details again IF they weren't loaded before (optional, maybe not needed)
          // For now, just re-render with existing TMDb data (if any)
          let existingTmdbDetails = null; // Need to somehow retrieve or know if TMDb was fetched for this item
          // Simplification: Assume tmdbDetails are stored globally or re-fetched if needed.
          // Re-rendering using the global currentItemDetailData which now has the new URL
          const actionHTML = createItemDetailContentHTML(currentItemDetailData, /* pass existing/cached tmdbDetails if available */ null); // Pass null for now
    ...
    // --- Event Delegation Setup ---
     function handleContentClick(event) { // Main delegation function
         ...
         // 2. Check for action clicks within the Item Detail view or Player controls
          // Include player controls except close and the player's own media controls (seek/vol/etc)
         const actionTrigger = target.closest('#item-detail-content .button, #videoContainer .button:not(.close-btn)');
          if (actionTrigger && !actionTrigger.closest('.custom-controls')) { // Exclude native player controls container
    ...
    document.addEventListener('DOMContentLoaded', async () => {
         ...
         if (videoContainer) { videoContainer.addEventListener('click', handleContentClick); } // Handles clicks inside player (e.g., custom URL play)
         ...
         document.addEventListener('click', (event) => {
             ...
             if (videoContainer && videoContainer.style.display !== 'none' && !isGlobalCustomUrlMode) {
                 const clickedInsidePlayer = videoContainer.contains(event.target);
                  // Check if click is inside the detail view *content* area where player might be
                 const clickedInsideDetailContent = itemDetailContent && itemDetailContent.contains(event.target);

                 // Click is outside the player AND outside the detail content area it resides in
         ...
         if(videoElement) {
             ...
             videoElement.removeEventListener('error', handleVideoError); // Remove previous listener just in case
             videoElement.addEventListener('error', handleVideoError); // Add the error handler
         }
